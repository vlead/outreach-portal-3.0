...
class DeleteUser():
    ...
    def auth_check(args):
        ...
        if System.is_session_admin(session):
            ...
            role = filter (lambda x: x.get('name') == "NCC" and x.get('centre').get("hub") == session.get("role").centre, user.get('roles'))
            if role:
                return True
        else:
            raise AuthError("Role not Authorized to perform this action")
...
class TestDeleteUser(TestCase):
    ...
    def test_delete_user_by_occ(self):
        ...
        oc_role = filter(lambda x:x.get('name') == "OCC" and x.get('centre').get("institute").get("name") == institute.get("name"), System.role_set)
        if oc_role:
        ...
        nc_role = filter(lambda x:x.get('name') == "NCC" and x.get('centre').get("institute").get("name") ==institute.get("name"), System.role_set)
        if nc_role:
...
class AddRole():
    ...
    def state_check(args):
        ...
        if System.is_session_OCC(session):
            if not role.get('name') == 'NCC':
                raise StateError("AddRole %s not allowed" %role.get('name'))
            else:
                if not (session.get('role').get('centre') == role.get('centre').get('hub')):
                    raise StateError("AddRole %s not allowed" %role.get('name'))
...
class CreateNC():
    ...
    def auth_check(args):
        ...
        if not (System.is_session_admin(session) or System.is_session_OCC(session)):
            raise AuthError("Role not Authorized to perform this action")
        if System.is_session_OCC(session):
            if oc != session.get('role').get('centre'):
                raise AuthError("Role not Authorized to perform this action")
...
ops_table = {'add_user' : AddUser,'login':login,'delete_user':DeleteUser,'logout':logout,'create_institute':CreateInstitute,'create_oc':CreateOC,'create_nc':CreateNC,'update_user':UpdateUser,'add_role':AddRole}                
#+end_src
** Test CreateNC
#+NAME: test_CreateNC
#+BEGIN_SRC python
