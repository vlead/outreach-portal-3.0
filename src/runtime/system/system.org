...
class AddRole():
    ...
    def state_check(args):
        ...
        if not (session in System.session_set):
            raise StateError("user not in session %s" % session.get('user').get('name').get('name'))
        if System.is_session_admin(session):
            if not System.handler.role_exists(role):
...
class TestAddRole(TestCase):
    ...
    def test_add_role(self):
        ...
        us = System.do("add_role",user=login_user,role=Role.noc,session=sess)
        self.assertTrue(len(us.get('roles'))==3)
...
class CreateInstitute():
    ...
    def state_check(args):
        ...
        for inst in System.handler.institute_set:
            if inst.get('name') == name and inst.get('address') == address:
                raise StateError("institute %s already in System" % inst.get('name'))
    ...
    def action(args):
        ...
        System.handler.add_institute(new_institute)
...
class TestCreateInstitute(TestCase):
    ...
    def test_create_institute(self):
        ...
        self.assertEqual(institute,System.handler.institute_set[0])
...
class CreateOC():
    ...
    def state_check(args):
        ...
        if not (session in System.session_set):
            raise StateError("user not in session %s" % session.get('user').get('name').get('name'))
        if not System.handler.institute_exists(institute):
    ...
    def action(args):
        ...
        System.handler.add_oc(new_oc)
        new_role = Role(name="OCC",centre_oc=new_oc,centre_nc=None)
        System.handler.add_role(new_role)
        return new_oc
#+end_src
** Test create_oc
#+NAME: test_CreateOC
...
class CreateWorkshop():
    ...
    def action(args):
        ...
        status = args["status"]
        nc = args['nc']
        new_workshop = Workshop(institute = institute, name = name, ws_target = ws_target, artefacts = artefacts, status = status, nc = nc)
        print new_workshop.get("name").get("name")
        System.handler.workshop_set.append(new_workshop)
...
import datetime
from runtime.system.object_handlers import *
#+end_src
