...
class DeleteUser():
    ...
    def auth_check(args):
        ...
        if System.is_session_admin(session):
            ...
            role = filter (lambda x: x.get('name') == "NCC" and x.centre.get("hub") == session.get("role").centre, user.roles)
            if role:
                return True
        else:
            raise AuthError("Role not Authorized to perform this action")
...
class TestDeleteUser(TestCase):
    ...
    def test_delete_user_by_occ(self):
        ...
        oc_role = filter(lambda x:x.get('name') == "OCC" and x.centre.get("institute").get("name") == institute.get("name"), System.role_set)
        if oc_role:
        ...
        nc_role = filter(lambda x:x.get('name') == "NCC" and x.centre.get("institute").get("name") ==institute.get("name"), System.role_set)
        if nc_role:
