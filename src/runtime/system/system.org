...
class CreateInstitute():
    ...
    def state_check(args):
        ...
        if not (session in System.session_set):
            raise StateError("user not in session %s"
                              % session.get('user').get('name').get('name'))
        for inst in System.delegate.institute_exists(institute):
...
#+begin_src python
class CreateOC():
    arg_types = {"institute":is_institute,"spokes":are_spokes_or_empty_list,
                  "session":is_session, "oc_target":is_oc_target}
    arity_and_type_checks_needed = True
    @staticmethod
    def auth_check(args):
        session = args['session']
...
class CreateNC():
    arg_types = {"institute":is_institute,"hub":is_oc,"session":is_session,
                  "nc_target" : is_nc_target, "workshops": are_workshops_or_empty_list}
    arity_and_type_checks_needed = True
    @staticmethod
    def auth_check(args):
        session = args['session']
...
from runtime.objects.entities import is_user, is_session, is_role, is_email, \
    is_name, is_alphabetic_str, is_str, are_spokes_or_empty_list, is_institute,\
    is_oc_target, is_oc, is_nc_target, are_workshops_or_empty_list, is_ws_target,\
    are_artefacts_or_empty_list, is_status, is_nc, is_workshop_or_none, is_ws_target, \
    is_artefact, Session
