...
class CreateOC():
    ...
    def state_check(args):
        ...
        for nc in System.nc_set:
            if nc.get('institute') == institute:
                raise StateError("institute %s already has an oc associated with it" %nc)
    ...
    def action(args):
        ...
        return new_oc
ops_table = {'add_user' : AddUser,'login':login,'delete_user':DeleteUser,'logout':logout,'create_institute':CreateInstitute,'create_oc':CreateOC,'update_user':UpdateUser}
