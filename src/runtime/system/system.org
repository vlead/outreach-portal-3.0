...
class CreateInstitute():
    ...
    def state_check(args):
        ...
        for inst in System.institute_set:
            if inst.name == name and inst.address == address:
...
ops_table = {'add_user' : AddUser,'login':login,'logout':logout,'create_institute':CreateInstitute, 'delete_user' : DeleteUser, 'update_user' : UpdateUser}
#+end_src
