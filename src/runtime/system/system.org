...
class AddUser():
    ...
    def action(args):
        ...
        user.set(roles=[Role(name="guest",centre=None)])
        return user
#+end_src
*** Test AddUser
#+NAME: test_adduser
...
class TestDeleteUser(TestCase):
    ...
    def test_delete_user_by_occ(self):
        ...
        oc_user=User(name=Name(name="maryoc"),email=Email(email="mary@gmail.com"))
        
        
        oc_role = filter(lambda x:x.name == "OCC" and x.centre.get("institute").get("name") == institute.get("name"), System.role_set)
        if oc_role:
            oc_user.set_role(oc_role[0])

        nc = System.do("create_nc",institute=institute,hub=oc,session=session)

        nc_user=User(name=Name(name="xyznc"),email=Email(email="xyz@gmail.com"))

        nc_role = filter(lambda x:x.name == "NCC" and x.centre.get("institute").get("name") ==institute.get("name"), System.role_set)
        if nc_role:
            nc_user.set_role(nc_role[0])
...
class TestUpdateUser(TestCase):
    ...
    def test_update_user_by_occ(self):
        ...
        oc_user=User(name=Name(name="useroc"),email=Email(email="useroc@gmail.com"))
        ...
        #create nc
        nc = System.do("create_nc",institute=institute,hub=oc,session=session)
        nc_user=User(name=Name(name="usernc"),email=Email(email="usernc@gmail.com"))
