...
class CreateOC():
    arg_types = {"institute":is_institute, "spokes":are_spokes_or_empty_list,
                  "session":is_session, "oc_targets":are_oc_targets_or_empty_list}
    arity_and_type_checks_needed = True
    ...
    def state_check(args):
        ...
        for oc in System.delegate.get_ocs():
            if oc.get('institute') == institute:
                raise StateError("institute %s already has an oc associated with it"
                                     %oc)
        for nc in System.delegate.get_ncs():
            if nc.get('institute') == institute:
                raise StateError("institute %s already has an nc associated with it"
                                     %nc)
    ...
    def action(args):
        institute = args["institute"]
        spokes = args["spokes"]
        oc_targets = args["oc_targets"]
        new_oc = System.delegate.add_oc(institute, spokes, oc_targets)
...
from runtime.objects.entities import is_user, is_session, is_role, is_email, \
    ...
    is_artefact, are_oc_targets_or_empty_list, Session
