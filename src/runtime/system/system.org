...
class CreateInstitute():
    ...
    def state_check(args):
        ...
        for inst in System.delegate.get_institutes():
            if inst.get('name') == institute.get('name') and inst.get('address') == institute.get('address'):
    ...
    def action(args):
        institute = args["institute"]
        System.delegate.add_institute(institute)
        return institute
#+end_src
...
class CreateOC():
    ...
    def action(args):
        ...
        spokes=args["spokes"]
        oc_target = args["oc_target"]
        new_oc = System.delegate.add_oc(institute, spokes, oc_target)
        new_role = System.delegate.add_occ_role("OCC", new_oc, None)
        return new_oc
#+end_src
...
class CreateWorkshop():
    ...
    def action(args):
        ...
        workshop = System.delegate.add_workshop(institute, name, ws_target, artefacts, status, nc)
        return workshop
#+end_src
...
from runtime.objects.entities import is_user, is_session, is_role, is_email, \
    is_name, is_alphabetic_str, is_str, are_spokes, is_institute, is_octarget, \
    is_oc, is_nctarget, are_workshops, is_wstarget, are_artefacts, is_status, is_nc, \
    is_workshop, is_wst, is_artefact, Session, OC, NC
