...
class System ():
    admin_user = User(name=Name(name=Config.admin_name),
                      ...
                      roles=[Role.admin,Role.guest,Role.noc],user_status="active")
    user_set = [admin_user]
    active_user_set=[admin_user]
    session_set = []
...
* =AddUser=
  *AddUser* operation takes in a =user= object and =session= object as parameter and
  adds the user to the =user-set= only if the following checks are true:
  1) *auth_check* : =session= object should be present in =session-set= and 
  the role name corresponding to the session should be either admin, NOC or OCC
  2) *state_check* : This check verifies if the =user= is not present in the system.
  If all the  above checks are passed then the following =action= s will be performed
  1) Adds  the  user to the =user-set= and
  2) Sets the default =Guest= role to that user.
#+NAME: class_adduser
#+begin_src python
class AddUser():
    ...
    def state_check(args):
        ...
        if (user in System.user_set):
            raise StateError("user %s already in System"
...
* =DeleteUser=
*DeleteUser* operation takes in a =user= object and =session= object as parameter and
sets the =user_status= to =inactive= only if the following checks are true:
1) *auth_check* : =session= object should be present in =session-set= and 
the role name corresponding to the session should be either admin, NOC or OCC. 
2) *state_check* : This check verifies if the =user= is already present in the system.
If all the  above checks are passed then the following =action=  will be performed
1) Sets the  =user_status= to =inactive=.
...
class TestDeleteUser(TestCase):
    ...
    def test_delete_admin_user(self):
        ...
        other_admin_user=User(name=Name(name="me"),
                              email=Email(email="abc@gmail.com"),
                              roles=[],user_status="active")
    ...
    def test_delete_user_by_occ(self):
        ...
        oc_user=User(name=Name(name="maryoc"),
                     email=Email(email="mary@gmail.com"),
                     roles=[],user_status="active")
        ...
        nc_user=User(name=Name(name="xyznc"),
    ...
    def test_delete_user_by_noc(self):
        ...
        institute = System.do("create_institute", name="IIT",
                               address="Delhi", session=session)
        oc = System.do("create_oc", institute=institute, session=session,
                        spokes=[], oc_target = None)
        ...
        noc_user=User(name=Name(name="xyznc"),
                      email=Email(email="xyz@gmail.com"),
                      roles=[],user_status="active")
...
#+END_SRC
* =UpdateUser=
*UpdateUser* operation takes in a =user= object, =session= object, =name=
object and =email= objects as parameter and updates the =name= or =email= or
both to the =user= only if the following checks are true:
1) *auth_check* : =session= object should be present in =session-set= and 
the role name corresponding to the session should be either admin, NOC or OCC. 
2) *state_check* : This check verifies if the =user= is already present in the system.
If all the  above checks are passed then the following =action=  will be performed
1) updates the =name= or =email= or both to the =user=

*Note:* Since this function takes in either name, email or both,
=arity_and_type_checks_needed= variable is set to =False=
...
class TestUpdateUser(TestCase):
    ...
    def test_update_user_by_admin(self):
        ...
        other_user=User(name=Name(name="me"),
                        email=Email(email="abc@gmail.com"),
                        roles=[],user_status="active")
        System.do("add_user", user=other_user, session=session)

        System.do("update_user",name=Name(name="john"),
    ...
    def test_update_user_by_occ(self):
        ...
        institute = System.do("create_institute",
                             name="IIT",address="Madras",session=session)
        oc = System.do("create_oc", institute=institute, session=session,
                        spokes=[], oc_target=None)
       
        oc_user=User(name=Name(name="useroc"),
        ...
        if oc_role:
            oc_user.append_role(oc_role[0])
        #create nc
        nc = System.do("create_nc", institute=institute, hub=oc,
                        session=session, nc_target = None, workshops = [])
        nc_user=User(name=Name(name="usernc"),
                     email=Email(email="usernc@gmail.com"),
                     roles=[],user_status="active")
    ...
    def test_update_user_by_noc(self):
        ...
        institute = System.do("create_institute",name="IIT",
                              address="Madras",session=session)
        oc = System.do("create_oc",institute=institute,session=session,
                        spokes=[], oc_target = None)
       
        oc_user=User(name=Name(name="useroc"),
                     email=Email(email="useroc@gmail.com"),roles=[],
                     user_status="active")
        ...
        noc_user=User(name=Name(name="usernoc"),
                      email=Email(email="usernoc@gmail.com"),roles=[],
                      user_status="active")
...
#+END_SRC
* =AddRole=
  *AddRole* operation takes in a =user= object, =session= object and a =role=
  object as parameter and adds the =role= to the =role-set= of the =user= only
  if the following checks are true:
  1) *auth_check* : =session= object should be present in =session-set= and 
  the role name corresponding to the session should be either admin, NOC or OCC
  2) *state_check* : This function verifies the following checks: 
     1) If the role of the current session is admin, then this operation
        is allowed and any role can be assigned to the given =user= only if
        that role exists in the =role-set=
     2) If the role of the current session is NOC, then this operation
        is allowed only if the name of the role is =OCC= and also the role
        should exists in the =role-set=.
     3) If the role name of the current session is OCC, then this operation is
        allowed only if the name of the role is =NCC= and the hub of
        =centre_nc= is same as =centre_oc= (of current session role).
        
  If all the  above checks are passed then the following =action= will be performed
  1) Adds  the role to the =role-set= of the =user= 
...
class TestAddRole(TestCase):
    ...
    def test_add_role(self):
        ...
        us = System.do("add_role", user=login_user,
                        role=Role.reviewer,session=sess)
...
class TestCreateInstitute(TestCase):
    ...
    def test_create_institute(self):
        ...
        institute = System.do("create_institute",name="IIIT",
                               address="Hyderabad",session=sess)
...
class TestCreateOC(TestCase):
    ...
    def test_create_oc(self):
        ...
        #self.assertRaises(StateError,System.do,"create_oc",
        #                  institute=institute,session=sess)
#+END_SRC
...
class CreateWorkshop():
    arg_types = {"institute":is_institute, "name" : is_name ,
                 "session":is_session, "ws_target" : is_wstarget,
                 "artefacts": are_artefacts, "status" : is_status,
                 "nc" : is_nc}
    arity_and_type_checks_needed = True
    @staticmethod
    def auth_check(args):
    ...
    def action(args):
        ...
        status = args["status"]
        nc = args['nc']
        new_workshop = Workshop(institute = institute, name = name,
                                ws_target = ws_target, artefacts = artefacts,
                                status = status)
...
class TestRescheduleWorkshop(TestCase):
    ...
    def test_reschedule_workshop(self):
        ...
        new_wstarget = WSTarget(usage = 3000, participants = 200,
                                experiments = 20, date = new_target_date)
...
class TestConductWorkshop(TestCase):
    ...
    def test_conduct_workshop(self):
        ...
        institute = System.do("create_institute",name="IIIT",
                               address="Hyderabad",session=session)
        workshop_institute = System.do("create_institute", name = "NITK",
                                        address = "Surathkal", session = session)
        oc = System.do("create_oc",institute=institute,spokes=[],
                        session=session, oc_target = None)
        nc = System.do("create_nc",institute=institute,hub=oc,session=session,
                        nc_target = None, workshops = [])
...
class UploadArtefact():
    ...
    def state_check(args):
        ...
        session = args["session"]
        workshop_status = workshop.get("status")
        if not (Status.is_completed(workshop_status) or
...
class TestApproveWorkshop(TestCase):
    ...
    def test_approve_workshop(self):
        ...
        System.do("approve_workshop", workshop = new_workshop, session = oc_session)
        self.assertEqual(new_workshop.get("status").get("name"), Status.approved.get("name"))
...
class TestDeleteArtefact(TestCase):
    ...
    def test_delete_artefact(self):
        ...
        oc = System.do("create_oc",institute=institute,spokes=[],
                        session=session, oc_target = None)
        nc = System.do("create_nc", institute=institute,
                        hub=oc,session=session, nc_target = None,
                        workshops = [])
        ...
        ws_status = Status.pending
        nc_user=User(name=Name(name="ncusernew"),
                     email=Email(email="ncusernew@gmail.com"),
                     roles=[],user_status="active")
        nc_user.set(roles=[Role(name = "NCC", centre_oc = oc, centre_nc = nc)])
        System.do("add_user",user=nc_user,session=session)
        nc_session = Session(user = nc_user,
...
#+begin_src python
class MakeUser():
    arg_types = {"name":is_str, "email":is_str, "session":is_session}
    arity_and_type_checks_needed = True 
    ...
    def action(args):
        ...
        name = args["name"]
        email = args["email"]
        u = User(name=Name(name=name),email=Email(email=email),
                 roles=[],user_status="active")
        System.do("add_user",user=u,session=session)
...
class TestMakeUser(TestCase):
    ...
    def tearDown(self):
        ...
        System.oc_set = []
        System.nc_set = []
    def test_make_user_by_admin(self):
        print "test_make_user_by_admin"
        admin_user=System.user_set[0]
        session = Session(user=admin_user,role=Role.admin)
        System.session_set.append(session)
 
        System.do("make_user",name="me",email="abc@gmail.com",session=session)
