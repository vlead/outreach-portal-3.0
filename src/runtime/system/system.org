...
1) auth_check : =session= object should be present in =session-set= and the role name corresponding
to the session should be either admin, NOC or OCC
Also there is a state_check to verify if the user is already present in the system.
