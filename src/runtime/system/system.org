...
class DeleteUser():
    ...
    def auth_check(args):
        ...
        if System.is_session_admin(session):
            ...
            role = filter (lambda x: x.get_name() == "NCC" and x.centre.get("hub") == session.get("role").centre, user.roles)
            if role:
                return True
        else:
            raise AuthError("Role not Authorized to perform this action")
...
class TestDeleteUser(TestCase):
    ...
    def test_delete_user_by_occ(self):
        ...
        oc_user=User(name=Name(name="me"),email=Email(email="abc@gmail.com"))

        oc_role = filter(lambda x:x.name == "OCC" and x.centre.get("name") ==institute.get("name"), System.role_set)
        if oc_role:
            oc_user.set_role(oc_role[0])
        #create nc
        nc_user=User(name=Name(name="me"),email=Email(email="abc@gmail.com"))

        nc_role = filter(lambda x:x.name == "NCC" and x.centre.get("name") ==institute.get("name"), System.role_set)
        if nc_role:
            nc_user.set_role(oc_role[0])

       
        System.do("add_user", user=oc_user, session=session)
        System.do("add_user", user=nc_user, session=session)

        oc_session = Session(user=oc_user,role=oc_role)
...
class TestUpdateUser(TestCase):
    ...
    def test_update_user_by_admin(self):
        ...
        other_user=User(name=Name(name="me"),email=Email(email="abc@gmail.com"))
    ...
    def test_update_user_by_occ(self):
        ...
        oc_user=User(name=Name(name="useroc"),email=Email(email="abc@gmail.com"))

        oc_role = filter(lambda x:x.name == "OCC" and x.centre.get("name") ==institute.get("name"), System.role_set)
        if oc_role:
            oc_user.set_role(oc_role[0])
        #create nc
        nc_user=User(name=Name(name="usernc"),email=Email(email="abc@gmail.com"))
        ...
        length = len(System.user_set)
        System.do("update_user", name="jack", user=nc_user,session=oc_session)
        new_length = len(System.user_set)
...
class logout():
    ...
    def state_check(args):
        session = args["session"]
        if not (session in System.session_set):
            raise StateError("user not in session %s" % session.user.name.name)
    ...
    def state_check(args):
        ...
        for inst in System.institute_set:
            if inst.get('name') == name and inst.get('address') == address:
                raise StateError("institute %s already in System" % institute.name)
...
class CreateOC():
    ...
    def state_check(args):
        ...
        if institute not in System.institute_set:
            raise StateError("institute %s not in System" % institute.name)
    ...
    def action(args):
        institute = args["institute"]
        new_oc = OC(institute=institute)
        System.oc_set.append(new_oc)
        new_role = Role(name="OCC",centre=new_oc)
...
class TestCreateOC(TestCase):
    ...
    def test_create_oc(self):
        ...
        sess = System.do("login",user=login_user,role=login_role)
        institute = System.do("create_institute",name="IIIT",address="Hyderabad",session=sess)
        oc = System.do("create_oc",institute=institute,session=sess)
...
class CreateNC():
    ...
    def auth_check(args):
        ...
        if System.is_session_OCC(session):
            if oc != session.get('role').get_centre():
                raise AuthError("Role not Authorized to perform this action")
    ...
    def action(args):
        ...
        new_role = Role(name="NCC",centre=new_nc)
        System.role_set.append(new_role)
        return new_nc

ops_table = {'add_user' : AddUser,'login':login,'delete_user':DeleteUser,'logout':logout,'create_institute':CreateInstitute,'create_oc':CreateOC,'create_nc':CreateNC,'update_user':UpdateUser}                
#+end_src
** Test CreateNC
#+NAME: test_CreateNC
#+BEGIN_SRC python
