...
#+BEGIN_SRC python
    def user_exists(self, user):
        ...
        try:
            user = User.apply_filters(email=email)[0]
            if user:
                return True
...
#+BEGIN_SRC python
    ...
    def test_add_user2(self):
        print "test_add_user2"
        name1 = Name(name="some user")
        email1 = Email(email="tt@kk.com")
        role1 = Role(name="admin", centre_nc=None, centre_oc=None)
        role2 = Role(name="noc", centre_nc=None, centre_oc=None)

        user1 = User(name=name1,
                    email=email1,
                    roles=[role1, role2])

        name2 = Name(name="some other user")
        email2 = Email(email="other@kk.com")
        role2 = Role(name="admin", centre_nc=None, centre_oc=None)

        user2 = User(name=name2,
                    email=email2,
                    roles=[role2])
        user1 = self.persistent_delegate.add_user(user1)
        user2 = self.persistent_delegate.add_user(user2)
        self.assertEqual(self.persistent_delegate.user_exists(user1), True)
        self.assertEqual(self.persistent_delegate.user_exists(user2), True)
...
#+BEGIN_SRC python
    def institute_exists(self, institute):
        ...
        try:
            institute = Institute.apply_filters(name=institute_name)[0]
            if institute:
                return True
        except NotFoundError as e:
            return False
...
#+BEGIN_SRC python
    def test_oc_exists(self):
         ...
         spokes = []
         oc_targets = []
         oc = self.persistent_delegate.add_oc(institute, spokes,
                                               oc_targets)
...
#+BEGIN_SRC python
    def add_oc(self, institute, spokes, oc_targets):
        institute.save()
        for spoke in spokes:
            spoke.save()
        for oc_target in oc_targets:
            oc_target.save()
        self.new_oc = OC(institute=institute, spokes=spokes, oc_targets=oc_targets)
        self.new_oc.save()
        return self.new_oc
#+END_SRC
...
#+BEGIN_SRC python
    def test_add_oc(self):
         ...
         spokes = []
         oc_targets = []
         oc = self.persistent_delegate.add_oc(institute, spokes, oc_targets)
         self.assertEqual(self.persistent_delegate.oc_exists(oc), True)
#+END_SRC
