...
class TestOC(TestCase):
    ...
    def test_sets_institute_on_OC(self):
        print "test_sets_institute_on_oc"
        oc = OC(institute=Institute(name="IIITH",address="Hyderabad"))
        self.assertEqual((oc.get("institute")).get("name"), "IIITH")
        ins = Institute(name="MEC",address="Hyderabad")
        oc.set(institute=ins)
        self.assertEqual(oc.get('institute').get("name"), "MEC")
#+END_SRC
