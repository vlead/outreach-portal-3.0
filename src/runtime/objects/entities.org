...
#+BEGIN_SRC python
def is_centre(value):
    if isinstance(value, OC) or isinstance(value,NC) or isinstance(value,None):
        return value
    else:
        raise TypeError("Invalid centre type")
Role = ClassTemplate.mk_class("Role", name=is_alphabetic_str,centre=is_centre)
is_role = is_inst(Role)
check_role = check_pred(is_role)
Role.getcentre = lambda self: \
                  return self.centre
Role.setcentre = lambda self, centre: \
                  if is_centre(centre): \
                  self.centre = centre

Role.admin = Role(name="admin")
Role.reviewer = Role(name="reviewer")
Role.guest = Role(name="guest")
...
#+NAME: class_Session
#+BEGIN_SRC python
Session = ClassTemplate.mk_class("Session", user=is_user)
