...
#+NAME: class_WSTarget
#+BEGIN_SRC python
WSTarget = ClassTemplate.mk_class("WSTarget", usage = is_int,
                                   participants = is_int,
                                   experiments = is_int, date = is_date)
...
NCTarget = ClassTemplate.mk_class("NCTarget", usage = is_int, date = is_date,
                                   wstargets = are_wstargets )
...
class TestNCTarget(TestCase):
    ...
    def test_instantiate_nctarget(self):
        ...
        date_format = "%d-%m-%Y"
        nctarget_check = NCTarget(usage=300, wstargets = [],
...
OCTarget = ClassTemplate.mk_class("OCTarget", usage = is_int, date = is_date,
                                   nctargets = are_nctargets )
...
class TestOCTarget(TestCase):
    ...
    def test_instantiate_octarget(self):
        ...
        date_check = "30-06-2016"
        date_format = "%d-%m-%Y"
        octarget_check = OCTarget(usage=300, nctargets = [],
                                  date = datetime.datetime.strptime(date_check, date_format).date())
...
* OC
  A =OC= entity has fields =institute= of type =is_institute=,
  =spokes= of type =are_sokes= and =oc_target= of type =is_octarget=
  Here spokes of an OC are all the nodal centers under it.
...
class TestOC(TestCase):
    ...
    def test_sets_spokes_on_OC(self):
        ...
        nc = NC(institute=Institute(name="HCU",address="Hyderabad"),
                hub=oc, nc_target = None, workshops = [])
...
* NC
  A =NC= entity has fields =institute= of type =Institute=,
  =hub= of type =is_oc= , =nc_target= of type =is_nctarget= , =workshops= of
  type =are_workshops=.
  The hub of a NC is the OC which it is under.
...
class TestNC(TestCase):
    ...
    def test_instatiate_NC(self):
        ...
        nc = NC(institute=Institute(name="HCU",address="Hyderabad"),
                ...
                spokes=[], oc_target = None), nc_target = None, workshops = [])
...
* Role
   ...
   =centre_oc= of type =is_centre_oc= , =centre_nc= of type =is_centre_nc=
   centre_oc will hold an outreach center corresponding to the role name OCC
   (Outreach-Center-Coordinator)
   Similarly centre_nc will hold a nodal center corresponding to the role name NCC.
   Other than OCC and NCC role names, for all the other role names centre_oc
   and centre_nc will be None.

   
  is_centre_oc takes in a value and checks if that is an instance of OC class
  and returns true, else it returns false.
  
  is_centre_nc takes in a value and checks if that is an instance of NC class
  and returns true, else it returns false.
...
class TestRole(TestCase):
    ...
    def test_instantiate_role(self):
        ...
        self.assertEqual(is_role(Role(name="admin", centre_oc=None,
                         centre_nc=None)), True)
        a_role = Role(name="admin", centre_oc=None,centre_nc=None)
        self.assertEqual(check_role(a_role), a_role)
        self.assertEqual(Role(name="admin", centre_oc=None,centre_nc=None).get("name"),
                         "admin")
...
* User
  A =User= entity has fields =name= of type =Name=, =email= of type =Email= ,
  role of type =Role= and user_status of type =is_user_status=
 
  is_user_status is a function that returns true only if the parameter passed
...
append_role is a function that takes role as an input and appends it to the
field roles, to the user that invoked this function. 
...
is_institute is a function that checks if the given parameter is an instance of Institute.

#+NAME: class_Institute
#+BEGIN_SRC python

Institute = ClassTemplate.mk_class("Institute", name=is_alphabetic_str,
                                    address=is_str)
...
class TestInstitute(TestCase):
    ...
    def test_instantiate_institute(self):
        ...
        self.assertEqual(is_institute(Institute(name="IIIT",address="Hyderabad")),
                         True)
...
#+NAME: class_Artefact
#+BEGIN_SRC python
Artefact = ClassTemplate.mk_class("Artefact", name=is_alphabetic_str, path=is_string)
...
Workshop = ClassTemplate.mk_class("Workshop", name=is_name,
...
class TestWorkshop(TestCase):
    ...
    def test_instatiate_Workshop(self):
        ...
        workshop = Workshop(name=Name(name="Test workshop"),
                            ...
                            ws_target = wstarget_check, status = status, artefacts = [])
        self.assertEqual(is_wkshp(workshop), True)
#+END_SRC
  
   
* Session
   =Session= is an entity that has two fields =user= of type =is_user= and role
   of type =is_role=
   
   is_user is a function that checks if the given input parameter is an
   instance of User class.
   is_role checks if the given input parameter is and instance of Role class.
