...
class TestInstitute(TestCase):
    ...
    def test_instantiate_institute(self):
        ...
        self.assertEqual(is_institute(Institute(name="IIIT",
                                                    address="Hyderabad")),
                             True)
        institute_x = Institute(name="IIIT",address="Hyderabad")
        self.assertEqual(check_institute(institute_x), institute_x)
        self.assertEqual(check_institute(institute_x).get("name"), "IIIT")
...
class TestOC(TestCase):
    ...
    def test_instatiate_OC(self):
        ...
        self.assertEqual(check_oc(oc).get("institute").get("name"), "IIITH")
        self.assertRaises(TypeError, check_oc("Hello World"))
    ...
    def test_add_oc_target_to_OC(self):
        ...
        octarget_check = OCTarget(usage=300, nctargets=[], date=date)

        oc.add_oc_target(octarget_check)
        self.assertEqual(oc.get("oc_target").get("usage"), 300)
    ...
    def test_equality_of_oc(self):
        ...
        oc2 = OC(institute=Institute(name="IIITH",address="Hyderabad"),
                     spokes=[], oc_target = None)
        oc3 = OC(institute=Institute(name="IITD",address="Delhi"),spokes=[],
                     oc_target = None)
...
class TestSession(TestCase):
    ...
    def test_equality_on_session(self):
        ...
        role = Role(name="admin", centre_oc=None,centre_nc=None)
        role1 = Role(name="occ", centre_oc=None,centre_nc=None)

        user1 = User(name=Name(name="Jimi Hendrix"),
                    email=Email(email="jimi@gnu.org"),
                    roles=[role],user_status="active"
                   )
        user2 = User(name=Name(name="Jimi Hendrix"),
                    email=Email(email="jimi@gmail.org"),
                    roles=[role1],user_status="active"
                   )

        session1 = Session(user=user1, role=role)
        session2 = Session(user=user1, role=role)
...
OCTarget.__eq__ = lambda self, other: \
                  ...
                  self.get("nctargets") == other.get("nctargets") and \
...
class TestWorkshop(TestCase):
    ...
    def test_equality_on_workshop(self):
        ...
        workshop3 = Workshop(name=Name(name="Test workshop Three"),
                                 institute=Institute(name="IITD",address="Delhi"),
                                 nc = nc1,
                                 ws_target = wstarget_check,
                                 status = status,
                                 artefacts = [],
...
WSTarget = ClassTemplate.mk_class("WSTarget", usage=is_int, 
                                  participants=is_int, experiments=is_int, 
                                  date=is_date, nc_target=is_nctarget)
