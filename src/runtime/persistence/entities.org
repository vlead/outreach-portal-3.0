...
class TestArtefact(TestCase):
    ...
    def test_instantiate_artefact(self):
        ...
        artefact_check = Artefact(name = "photo", path = "/main/photos")
        artefact_check.save()
...
class TestWorkshop(TestCase):
    ...
    def test_instatiate_Workshop(self):
        ...
        wstarget_check.save()
        oc = OC(institute=Institute.get_by_id(1), spokes=[], oc_target = oc_target)
        oc.save()
        nc = NC(institute=Institute.get_by_id(2), hub=OC.get_by_id(1), nc_target = nc_target, workshops = [])
        nc.save()
